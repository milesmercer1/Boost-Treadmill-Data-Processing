...
for name = ["noBWS.xlsx", "0%.xlsx", "20%.xlsx", "40%.xlsx"]
    ...
    for sheet = ["t1", "t2", "t3","t4","t5","t6"]
        ...
        %forefoot
        %Rearfoot is column 2, Forefoot is column 3 for both feet once read
        %in
        

        %%%%switch to left foot if processing
        rearfoot_data = loadsol_data_leftfoot(:,1);
        forefoot_data = loadsol_data_leftfoot(:,2);
        %%%%change feet
        [data_row, data_col] = size(loadsol_data_leftfoot);
        %%%%    
        ...
        GRF_data = loadsol_data_leftfoot(1:6000,3);
        GRF_data_right = loadsol_data_rightfoot(1:6000,3);
        ...
        fprintf(' Click on the Impact Peak\n')
            ...
            %make sure to round values for easy interpretation
